<script setup lang="ts">
  ...
  const getModelTypeIcon = (category: string) => {
    ...
    if (
      ...
      categoryLower.includes("generation")
    ) {
    ...
    try {
      ...
      try {
        ...
        console.log(
          ...
          } logs for instance ID: ${instanceId || "N/A"}`,
        );
      ...
      const transformedLogs = (response.logs || []).map(
      ...
      logsError.value = `Error loading logs: ${
        err instanceof Error ? err.message : String(err)
      }`;
  ...
  const downloadLogs = () => {
    ...
    const exportFileDefaultName = `vm-logs-${vm.value?.name}-${new Date()
      .toISOString()
      .slice(0, 10)}.json`;
  ...
  const mockVmLogs = () => {
    ...
    const llmSpecificActions = [
      ...
      {
        severity: "DEBUG",
        message: "Tokenizer initialized with vocabulary size 32000",
      },
    ...
    const errorActions = [
      ...
      {
        severity: "WARNING",
        message: "Rate limiting applied to incoming requests",
      },
      {
        severity: "ERROR",
        message: "Failed to load configuration file at /etc/model-config.json",
      },
      {
        severity: "ERROR",
        message: "Out of memory error when loading large tensor batch",
      },
      { severity: "ERROR", message: "API request timed out after 30 seconds" },
      {
        severity: "WARNING",
        message: "Slow disk I/O detected - check storage performance",
      },
    ...
    const runtimeActions = [
      ...
      {
        severity: "WARNING",
        message: "Connection from unauthorized IP 198.51.100.123 blocked",
      },
      {
        severity: "INFO",
        message: "Serving model parameters: temperature=0.7, top_p=0.95",
      },
    ];
    ...
    const allActions = [
      ...
      ...Array(15)
        .fill(0)
        .flatMap(() => runtimeActions), // More runtime logs
      ...Array(3)
        .fill(0)
        .flatMap(() => errorActions), // Fewer error logs
    ];
    ...
    for (let i = 0; i < 5; i++) {
      const randomAction =
        runtimeActions[Math.floor(Math.random() * runtimeActions.length)];
    ...
    function createMockLog(action, timestampMs, id) {
      ...
      const insertId = crypto.randomUUID
        ? crypto.randomUUID()
        : `mock-log-${id}-${Date.now()}`;
      const traceId = `projects/airunner/traces/${Math.random()
        .toString(36)
        .substring(2, 15)}`;
      ...
      if (
        action.message.includes("Processing") ||
        action.message.includes("Request processed")
      ) {
        jsonPayload.request = {
  ...
  const downloadChat = () => {
    ...
    try {
      ...
      const exportFileName = `${vm.value?.name}-chat-${new Date()
        .toISOString()
        .slice(0, 10)}.json`;
      ...
      <div>
        ...
        <div v-if="activeTab === 'overview'" class="space-y-6">
          ...
          >
              ...
              >
                <div>
                  ...
                  <p class="text-gray-400 mt-1">
                    Real-time VM instance information
                  </p>
                </div>
                <div class="flex flex-col items-end">
                  <div class="flex items-center mb-2">
                    ...
                    <span class="ml-2 text-gray-400 text-xs"
                      >Instance ID: {{ vm.instanceId || "N/A" }}</span
                    >
                  </div>
                  <div class="flex space-x-2">
                    <span class="text-gray-400 text-xs"
                      >Last update: {{ formatDate(new Date()) }}</span
                    >
                    <button
                      @click="loadVmDetails"
            ...
            <!-- VM Stats Tiles -->
            <div
              class="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-800"
            >
              <div class="p-6">
                ...
                <p class="text-2xl font-bold text-gray-100 mt-2">
                  {{
                    vm.status === "RUNNING"
                      ? computeUptime(
                          vm.lastStatusUpdate || vm.creationTimestamp,
                        )
                      : "Not running"
                  }}
                </p>
              ...
              <div class="p-6">
                ...
                <p class="text-2xl font-bold text-gray-100 mt-2">
                  {{
                    vm.status === "RUNNING" && vm.apiEndpoint
                      ? "Available"
                      : "Unavailable"
                  }}
                </p>
              ...
              <div class="p-6">
                <div class="flex items-center justify-between">
                  ...
                  <component
                    :is="getModelTypeIcon(vm.modelDetails?.category)"
            ...
            <div class="p-6">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6">
                ...
                <div>
                  ...
                  <p
                    class="text-gray-100 font-medium text-lg flex items-center"
                  >
                    {{ vm.modelDetails?.company || "Custom" }}
                    <span
          ...
          >
            ...
            <div
              class="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-gray-800"
            >
              <div class="p-6">
                <div class="flex items-center mb-2">
                  <div
                    class="w-8 h-8 rounded-md bg-indigo-900/50 flex items-center justify-center mr-3"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                  ...
                  <div>
                    ...
                    <p class="text-gray-100 text-lg font-semibold">
                      {{ vm.specs?.cpu || "4" }} vCPUs
                    </p>
                  </div>
                </div>
                <div
                  class="mt-2 h-1.5 w-full bg-gray-700 rounded-full overflow-hidden"
                >
                  <div
              ...
              <div class="p-6">
                <div class="flex items-center mb-2">
                  <div
                    class="w-8 h-8 rounded-md bg-purple-900/50 flex items-center justify-center mr-3"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
              ...
              <div class="p-6">
                <div class="flex items-center mb-2">
                  <div
                    class="w-8 h-8 rounded-md bg-green-900/50 flex items-center justify-center mr-3"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                    ...
                    >
                      <path
                        d="M21 5c0-1.1-.9-2-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5Z"
                      ></path>
              ...
              <div class="p-6">
                <div class="flex items-center mb-2">
                  <div
                    class="w-8 h-8 rounded-md bg-yellow-900/50 flex items-center justify-center mr-3"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
          ...
          >
            <div class="p-6 border-b border-gray-800">
              ...
              <p class="text-gray-400 mt-1">
                Access your model through this endpoint
              </p>
            </div>
            ...
            <div class="p-6">
              <div class="mb-4">
                <div class="flex justify-between items-center mb-3">
                  <div class="flex items-center text-gray-400">
                    ...
                    >
                    <span
                      class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-900/50 text-green-300"
                      >ACTIVE</span
                    >
                  </div>
                  <div class="flex space-x-2">
                ...
                <div
                  class="bg-gray-800 p-4 rounded-md font-mono text-sm text-gray-200 break-all border border-gray-700"
                >
                ...
                <div
                  v-if="copySuccess"
                  class="mt-2 text-xs text-green-400 flex items-center"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
              ...
              <div class="mt-6 pt-6 border-t border-gray-800">
                ...
                <div
                  class="bg-gray-800 p-4 rounded-md font-mono text-xs text-gray-300 overflow-x-auto border border-gray-700"
                ></div>
              </div>
            </div>
          </div>
        ...
        <div v-else-if="activeTab === 'logs'" class="space-y-6">
          ...
          >
            ...
            <!-- Loading state -->
            <div
              v-if="isLoadingLogs && !vmLogs.length"
              class="py-16 text-center bg-gray-800"
            >
              <div class="relative w-16 h-16 mx-auto mb-4">
                <div
                  class="absolute inset-0 rounded-full border-4 border-t-indigo-400 border-r-indigo-500 border-b-indigo-600 border-l-indigo-400 animate-spin"
                ></div>
              </div>
                ...
                <table class="min-w-full divide-y divide-gray-800">
                  <thead class="bg-gray-800">
                    <tr>
                      <th
                        scope="col"
                        class="pl-6 pr-3 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider"
                      >
                        Time
                      </th>
                      <th
                      ...
                      <th
                        scope="col"
                        class="pl-3 pr-6 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider"
                      >
                        Actions
                      </th>
                    </tr>
                  ...
                  <tbody class="bg-gray-900 divide-y divide-gray-800">
                    <template v-for="log in vmLogs" :key="log.id">
                      <tr
                        class="hover:bg-gray-800/70 transition-colors duration-150"
                      >
                        <td
                          class="pl-6 pr-3 py-3 whitespace-nowrap text-xs text-gray-400"
                        >
                        ...
                        <td class="px-3 py-3 whitespace-nowrap">
                          ...
                          >
                            <component
                              :is="getLogSeverityIcon(log.severity)"
                              class="h-3 w-3 mr-1"
                            />
                            {{ log.severity }}
                          </span>
                        ...
                        <td
                          class="pl-3 pr-6 py-3 whitespace-nowrap text-right text-xs"
                        >
                          <button
                            @click="
                              showLogDetails =
                                showLogDetails === log.id ? null : log.id
                            "
                      ...
                      <tr
                        v-if="showLogDetails === log.id"
                        :key="`detail-${log.id}`"
                        class="bg-gray-800"
                      >
                        <td colspan="4" class="px-6 py-4">
                          <div class="text-sm">
                            <h4 class="font-medium text-gray-200 mb-3">
                              Log Details
                            </h4>
                            <div
                            ...
                            <div class="mb-4">
                              ...
                              <p
                                class="text-sm break-words bg-gray-900 p-3 rounded-md border border-gray-700 text-gray-300"
                              >
                                {{ log.message }}
                              </p>
                            </div>
                            <div>
                              ...
                              <pre
                                class="text-xs bg-gray-950 text-gray-300 p-4 rounded-md overflow-x-auto shadow-sm border border-gray-800"
                                >{{ JSON.stringify(log.json, null, 2) }}</pre
                              >
                            </div>
            ...
            <div v-else class="py-16 text-center bg-gray-800">
              <div
                class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-700 mb-4"
              >
            ...
            <!-- Load more logs -->
            <div
              v-if="hasMoreLogs && vmLogs.length > 0"
              class="bg-gray-900 border-t border-gray-800 p-4 text-center"
            >
              <button
        ...
        <div v-else-if="activeTab === 'features'" class="space-y-6">
          <div
            v-if="vm.status !== 'RUNNING'"
            class="bg-gray-900 shadow-xl rounded-lg overflow-hidden border border-gray-800 p-6 text-center"
          >
            <div
          ...
          <template v-else>
            ...
            >
              <div class="flex flex-col h-[600px]">
                ...
                >
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p
                        v-if="!selectedApiKeyId && apiKeys.length > 0"
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        />
                        <span class="text-xs text-gray-300 w-8">{{
                          temperature
                        }}</span>
                      </div>
                    </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-950">
                  ...
                  >
                    ...
                    >
                      ...
                      <div
                        v-if="!message.isTyping && message.role !== 'system'"
                ...
                <div
                  v-if="chatError"
                  class="p-3 bg-red-900/30 border-t border-red-800"
                >
            ...
            >
              <div class="p-6 border-b border-gray-800">
                <div class="flex items-center justify-between">
                  ...
                  <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-900/50 text-green-300"
                  >
                    RUNNING
                  </span>
                </div>
              ...
              <div class="p-6">
                ...
                >
                  <div
                    class="w-16 h-16 rounded-full bg-indigo-900/50 flex items-center justify-center mb-4"
                  >
                    <component
                      :is="getModelTypeIcon(vm.modelDetails?.category)"
                  ...
                  <p class="text-gray-400 max-w-md">
                    Support for
                    {{ vm.modelDetails?.category || "this model type" }} is
                    under development. Meanwhile, you can access this model
                    through the API endpoint directly.
                  </p>

                  <div class="mt-8 w-full max-w-2xl">
                    <div
                      class="bg-gray-800 p-4 rounded-md border border-gray-700"
                    >
                      <div class="flex justify-between items-center mb-3">
                        <div class="flex items-center text-gray-400">
                          <span class="text-xs uppercase tracking-wider"
                    ...
                    <div
                      v-if="copySuccess"
                      class="mt-2 text-xs text-green-400 flex items-center justify-center"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"

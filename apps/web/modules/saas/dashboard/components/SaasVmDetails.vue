  ...
  try {
    ...
    vm.value = {
      ...
      modelDetails: {
        ...
        description: "A deployed AI model instance running on your infrastructure.",
    ...
    error.value = `Error loading VM details: ${err instanceof Error ? err.message : String(err)}`;
  } finally {
...
const startVm = async () => {
  try {
    ...
    vmActionError.value = `Error starting VM: ${err instanceof Error ? err.message : String(err)}`;
    isStartingVm.value = false;
  }
};
...
const stopVm = async () => {
  try {
    ...
    vmActionError.value = `Error stopping VM: ${err instanceof Error ? err.message : String(err)}`;
    isStoppingVm.value = false;
  }
};
  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      ...
      <button
        ...
        :disabled="isLoading"
      >
        <RefreshCwIcon class="h-4 w-4 mr-1" :class="{ 'animate-spin': isLoading }" />
        Refresh
      </button>
    </div>
    ...
    <div v-else-if="vm" class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 border-b border-gray-200 pb-4">
        <div>
          <h1 class="text-2xl font-semibold flex items-center gap-2">
            <ServerIcon class="h-6 w-6 text-indigo-600" />
            {{ vm.name }}
          </h1>
          <p class="text-gray-500">{{ vm.labels?.model_name || "Custom Model" }}</p>
        </div>
      ...
      <div>
        ...
        <div v-if="activeTab === 'overview'" class="space-y-6">
          ...
          <div class="bg-white shadow rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p class="font-medium">{{ vm.labels?.model_name || "Custom Model" }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500">Company</p>
                <p class="font-medium">{{ vm.modelDetails?.company || "Custom" }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500">Category</p>
                <p class="font-medium">{{ vm.modelDetails?.category || "AI Model" }}</p>
              </div>
              <div>
                <p class="text-sm text-gray-500">Created</p>
                <p class="font-medium">{{ formatDate(vm.creationTimestamp) }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p class="font-medium">{{ formatDate(vm.lastStatusUpdate || vm.creationTimestamp) }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-if="vm.status === 'RUNNING'" class="bg-white shadow rounded-lg p-6">
            <h2 class="text-lg font-medium mb-4">API Endpoint</h2>
            <div class="mb-4">
              <p class="text-sm text-gray-500 mb-2">Endpoint URL</p>
              <div class="flex items-center">
                <div class="bg-gray-100 p-3 rounded-md font-mono text-sm break-all flex-grow">
                  {{ vm.apiEndpoint || `https://api.example.com/v1/models/${vm.name}` }}
                </div>
                <button
                  @click="copyToClipboard(vm.apiEndpoint || `https://api.example.com/v1/models/${vm.name}`)"
                  class="ml-2 p-2 text-gray-500 hover:text-indigo-600 rounded-md hover:bg-gray-100"
                  title="Copy to clipboard"
                >
                  <span v-if="copySuccess" class="text-green-500 text-xs font-medium">Copied!</span>
    ...
    "max_tokens": 100
        ...
        <div v-else-if="activeTab === 'logs'" class="space-y-6">
          <div class="bg-white shadow rounded-lg p-6">
            ...
            <div class="bg-gray-900 text-gray-300 p-4 rounded-md font-mono text-sm h-96 overflow-y-auto">
              <p v-for="(log, index) in vmLogs" :key="index" class="my-1">{{ log }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'performance'" class="space-y-6">
          <div v-if="vm.status !== 'RUNNING'" class="bg-white shadow rounded-lg p-6 text-center">
            <p class="text-gray-500">VM is not running. Start the VM to view performance metrics.</p>
            <button
              v-if="vm.status === 'TERMINATED'"
              @click="startVm"
              :disabled="isStartingVm"
            ...
            >
              <LoaderIcon v-if="isStartingVm" class="h-4 w-4 mr-1 animate-spin" />
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <div class="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                    <div class="bg-blue-600 h-2.5 rounded-full" :style="{ width: performanceData.cpu }"></div>
                  </div>
                </div>
                <div>
                  <p class="text-sm text-gray-500 mb-1">Memory Usage</p>
                  <p class="font-medium text-lg">{{ performanceData.memory }}</p>
                  <div class="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                    <div class="bg-purple-600 h-2.5 rounded-full" style="width: 51%"></div>
                  </div>
                </div>
                <div>
                  ...
                  <div class="w-full bg-gray-200 rounded-full h-2.5 mt-2">
                    <div class="bg-green-600 h-2.5 rounded-full" style="width: 34%"></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p class="font-medium text-lg">{{ performanceData.network.in }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-500">Outbound</p>
                  <p class="font-medium text-lg">{{ performanceData.network.out }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white shadow rounded-lg p-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <p class="font-medium text-lg">{{ performanceData.requests.total }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-500">Request Rate</p>
                  <p class="font-medium text-lg">{{ performanceData.requests.rate }}</p>
                </div>
                <div>
                  <p class="text-sm text-gray-500">Average Latency</p>
                  <p class="font-medium text-lg">{{ performanceData.requests.avgLatency }}</p>
                </div>
              </div>
            </div>
          </template>

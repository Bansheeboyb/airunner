...
export const sendVmMessage = protectedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    try {
      ...
      console.log(
        `Using X-API-Key authentication (key prefix: ${apiKey.substring(
          0,
          5,
        )}...)`,
      );
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          throw new TRPCError({
            code: "UNAUTHORIZED",
            message:
              "Unauthorized: The API key was rejected. Check that the API key is correct and active.",
          });
        } else if (response.status === 404) {
          throw new TRPCError({
            code: "NOT_FOUND",
            message:
              "The VM API endpoint could not be found. It may be unavailable or misconfigured.",
          });
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message:
              "The VM server encountered an error. Please try again later.",
          });
        } else {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: `Error from VM: ${
      ...
      try {
        ...
        console.log("Received raw response from Phi-4 API:", responseText);
      ...
      console.log("Parsed response from Phi-4 API:", result);

      // Create a direct passthrough of the original response
      // This ensures all fields from the LLM API are preserved exactly as is
      if (result && typeof result === "object") {
        // Check if this is the expected Phi-4 model response format
        if (result.generated_text !== undefined) {
          console.log("Detected Phi-4 API response format");

          // Also add a 'text' field for compatibility with client-side code
          ...
          console.log(
            "Returning response with text field added:",
            responseData,
          );
          return responseData;
        }
      ...
      console.log("Using fallback response parsing logic");
      ...
      // Standard response format as fallback
      const responseData = {
        text: responseText,
        usage: usage,
        model: modelInfo,
      ...
      console.log("Returning fallback response:", responseData);
      return responseData;
    } catch (error) {
